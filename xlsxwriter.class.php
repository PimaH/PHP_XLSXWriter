...
class XLSXWriter
{
	...
	protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false, $page_setup=array(), $auto_filter_margin = "")
	{
		...
		$this->sheets[$sheet_name] = (object)array(
			...
			'auto_filter_margin' => $auto_filter_margin,
    ...
    $style = &$col_options;
		...
		$auto_filter_margin = isset($col_options['auto_filter_margin']) ? (string)$col_options['auto_filter_margin'] : "A1:G100";
		...
		self::initializeSheet($sheet_name, $col_widths, $auto_filter, $freeze_rows, $freeze_columns, $page_setup, $auto_filter_margin);
		$sheet = &$this->sheets[$sheet_name];
	...
	public function set_allowed_value_func($key, $alloweed_array, $default, $page_setup)
	{
		return (isset($page_setup[$key]) && in_array($page_setup[$key], $alloweed_array)) ? $page_setup[$key] : $default;
	}
	public function set_boolean_value_func($key, $default, $page_setup)
	{
	...
	protected function finalizeSheet($sheet_name)
	{
		...
		if ($sheet->auto_filter) {
			$sheet->file_writer->write(	'<sheetProtection
		      sheet="false"
		      objects="false"
		      scenarios="false"
		      formatCells="false"
		      ...
		      selectUnlockedCells="false"/>
				<autoFilter ref="' . $sheet->auto_filter_margin . '"/>');
		}
		...
		$sheet->file_writer->write(      ' headings="'           . self::set_boolean_value_func('headings',            false, $page_setup) . '"');
		$sheet->file_writer->write(      ' gridLines="'          . self::set_boolean_value_func('grid_lines',          false, $page_setup) . '"');
		$sheet->file_writer->write(      ' gridLinesSet="'       . self::set_boolean_value_func('grid_lines_set',      true, $page_setup) . '"');
		$sheet->file_writer->write(      ' horizontalCentered="' . self::set_boolean_value_func('horizontal_centered', false, $page_setup) . '"');
		$sheet->file_writer->write(      ' verticalCentered="'   . self::set_boolean_value_func('vertical_centered',   false, $page_setup) . '"');
		...
		$sheet->file_writer->write(      ' pageOrder="'          . self::set_allowed_value_func('page_order', array('overThenDown', 'downThenOver'),  'downThenOver', $page_setup) . '"');
		$sheet->file_writer->write(      ' orientation="'        . self::set_allowed_value_func('orientation', array('landscape', 'portrait', 'default'),  'default', $page_setup) . '"');
		$sheet->file_writer->write(      ' usePrinterDefaults="' . self::set_boolean_value_func('use_printer_defaults',                                         true, $page_setup) . '"');
		$sheet->file_writer->write(      ' blackAndWhite="'      . self::set_boolean_value_func('black_and_white',                                             false, $page_setup) . '"');
		$sheet->file_writer->write(      ' draft="'              . self::set_boolean_value_func('draft',                                                       false, $page_setup) . '"');
		$sheet->file_writer->write(      ' cellComments="'       . self::set_allowed_value_func('cell_comments', array('asDisplayed', 'atEnd', 'none'),       'none', $page_setup) . '"');
		$sheet->file_writer->write(      ' useFirstPageNumber="' . self::set_boolean_value_func('use_first_page_number',                                       false, $page_setup) . '"');
		$sheet->file_writer->write(      ' errors="'             . self::set_allowed_value_func('errors', array('blank', 'dash', 'displayed', 'NA'),     'displayed', $page_setup) . '"');
	...
	private static function numberFormatStandardized($num_format)
	{
		...
		else if ($num_format=='euro')     $num_format='#,##0.00 [$ï¿½-407];[RED]-#,##0.00 [$ï¿½-407]';
		$ignore_until='';
		$escaped = '';
